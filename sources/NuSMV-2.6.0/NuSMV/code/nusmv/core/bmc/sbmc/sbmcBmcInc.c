                     ...
                     const int opt_do_completeness_check)
{
  ...
  for (current_k = 0; current_k <= max_k && !found_solution; current_k++) {
     ...
     */
    for (i = max_nusmv(previous_k + 2, 0); i <= current_k + 1; i++) {
     ...
     */
    for (i = max_nusmv(previous_k, 0); i < current_k; i++) {
     ...
     * Assumes that it has already been unrolled up to previous_k
     */
    for (i = max_nusmv(previous_k + 1, 0); i <= current_k; i++) {
     ...
     */
    for (i = max_nusmv(previous_k, 0); i <= current_k-1; i++) {
                            ...
                            Slist_ptr* conflict)
{
  ...
  for (current_k = 0; current_k <= max_k && !found_solution; current_k++) {
     ...
     * Assumes that they have already been build up to previous_k + 1
     */
    for (i = max_nusmv(previous_k + 1, 0); i <= current_k+1; i++) {
      be_ptr be_constraint = sbmc_build_InLoop_i(be_enc,
                                                 state_vars,
                                                 InLoop_array, i);
      if (be_constraint) {
